...
using System.Data;
using ThuongMaiDienTu.Areas.Admin.Models;
...
namespace ThuongMaiDienTu.Areas.Admin.Controllers
{
  ...
  public class CategoryController : Controller
  {
    ...
    public async Task<IActionResult> getList(jDatatable model, Guid? parentId)
    {
      ...
      int recordsTotal = 0;
      if (
          !string.IsNullOrEmpty(model.columns[model.order[0].column].name)
    ...
    {
      ThuongMaiDienTu.Models.Category item;
      if (model.Id == null)
      {
        item = new ThuongMaiDienTu.Models.Category();
    ...
    {
      var categoryInProduct = await _dbContext
        .Products.Where(m => m.CategoryId == id)
        .FirstOrDefaultAsync();
      if (categoryInProduct != null)
      {
        return Ok(false);
      }
      var categoryInRole = await _dbContext
        .Roles.Where(m => m.CategoryId == id)
        .FirstOrDefaultAsync();
      if (categoryInRole != null)
      {
        return Ok(false);
      }
      var item = await _dbContext.Categories.FindAsync(id);
